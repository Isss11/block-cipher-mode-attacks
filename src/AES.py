...
# If set to False, there is no parallel encryption with ECB
PARALLEL_ECB = False

# The following code (besides the ECB and CBC implementation) was copied from the following source (this is referenced in the report as well):
...
# Transforms data into a single stream from the grid representation
def convertToByteStream(grids):
    int_stream = []
    for grid in grids:
...
# Breaks into Blocks for Block Ciphers
def getBlockGrids(data):
...
# New ECB function to separate the logic between AES and the block ciphers
def ECB(key, data, encrypt=True):
    grids = getBlockGrids(data)

    # Expand the key for the multiple rounds (property of AES)
    expanded_key = expand_key(key, 11)

    tempGrids = []

    if encrypt:
        # Included parallel processing (if flag is set)
        if PARALLEL_ECB:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(enc, grid, expanded_key) for grid in grids]

            tempGrids = [future.result() for future in futures]
        else:
            for grid in grids:
                tempGrids.append(enc(grid, expanded_key))
    else:
        if PARALLEL_ECB:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(dec, grid, expanded_key) for grid in grids]

            tempGrids = [future.result() for future in futures]
        else:
            for grid in grids:
                tempGrids.append(dec(grid, expanded_key))
        
    grids = tempGrids
        
    return convertToByteStream(grids)

# CBC Mode of Operation Implementation
def CBC(key, iv, data, encrypt=True):
    grids = getBlockGrids(data)
    ...
    if encrypt:        
        for i in range(len(grids)):
            ...
            for column in range(4):
                for row in range(4):
                    int_stream.append(grids[i][row][column])
                    
            convertedPlaintext = bytes(int_stream)
            
            # XOR IV or previous ciphertext with plaintext
            if i == 0:  
                xorGrid = bytes([a ^ b for a, b in zip(convertedPlaintext, iv)])
            else:
                int_stream = []
                for column in range(4):
                    for row in range(4):
                        int_stream.append(tempGrids[-1][row][column])
                    
                previousCiphertext = bytes(int_stream)
                
                xorGrid = bytes([_a ^ _b for _a, _b in zip(convertedPlaintext, previousCiphertext)])
            
            tempGrids.append(enc(getBlockGrids(xorGrid)[0], expanded_key))
    # Decryption
    else:
        for i in range(len(grids)):
            # Run decryption function, then xor the value
            tempGrids.append(dec(grids[i], expanded_key))
            ...
            partiallyDecrypted = bytes(int_stream)
            
            # take partially decrypted ciphertext and then XOR it by the IV or the previous ciphertext
            if i == 0:
                xorGrid = bytes([_a ^ _b for _a, _b in zip(partiallyDecrypted, iv)])
            else:
                # Getting previous ciphertext as a byte string
                int_stream = []
                for column in range(4):
                    for row in range(4):
                        int_stream.append(grids[i - 1][row][column])
                        
                previousCiphertext = bytes(int_stream)
                
                xorGrid = bytes([_a ^ _b for _a, _b in zip(partiallyDecrypted, previousCiphertext)])
                
            tempGrids[-1] = getBlockGrids(xorGrid)[0]
...
def enc(passedGrid, expanded_key):
    grid = passedGrid
    temp_grid = grid
    ...
    # Applying to each grid separately in ECB mode
    temp_grid = add_sub_key(grid, round_key)
...
def dec(passedGrid, expanded_key):
    grid = passedGrid
    round_key = extract_key_for_round(expanded_key, 10)
